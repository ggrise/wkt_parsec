...
std_do([]) -> [];
std_do([{generate,_,{var,_,_},Val}]) ->
	% No point binding Var if this is the last statement
	[expr(Val)];
std_do([{generate,Line,Var,Val}|Rest]) ->
	[{call,Line,
		{'fun',Line,
			{clauses,
				[{clause,Line,
					 ...
					 std_do(Rest)}]}},
	[expr(Val)]}];
std_do([S|Ss]) -> [S|std_do(Ss)].


local_do(_, [{generate,_,{var,_,_},Val}]) ->
	% No point binding Var if this is the last statement
	expr(Val);
local_do(_, [Val]) -> expr(Val);
local_do(Bind, [{generate,Line,Var,Val}|Rest]) ->
	{call, Line, Bind,
	 [expr(Val), {'fun', Line, {clauses, [{clause, Line, [Var], [], [local_do(Bind, Rest)]}]}}]
	};
local_do(Bind, [E = {match,Line,_,_}|Rest]) -> {block, Line, [expr(E), local_do(Bind, Rest)]};
local_do(Bind, [S|Rest]) when is_tuple(S) ->
	Line = element(2, S),
	local_do(Bind, [{generate, Line, {var,Line,'_'}, S}|Rest]).
...
expr(Do = {lc, Line, {remote,_,{atom,_,do},{var,_,'_'}}, Qs0}) ->
	Verbose = get(verbose),
	if Verbose ->
			io:format("Line ~B:\n\n~s\n\n", [Line, prettypr:format(prettypr:beside(prettypr:text("\t"), erl_prettypr:layout(Do)))]);
		true -> []
		end,

    Qs1 = {block, Line, std_do(Qs0)},

	if Verbose ->
			io:format("Transformed to:\n\n~s\n", [prettypr:format(prettypr:beside(prettypr:text("\t"), erl_prettypr:layout(Qs1)))]),
			io:format("\n~s\n", [lists:duplicate(79, $-)]);
		true -> []
		end,
...
expr(Do = {lc, Line, {remote,_,{atom,_,do},Bind}, Qs0}) ->
	Verbose = get(verbose),
	if Verbose ->
			io:format("Line ~B:\n\n~s\n\n", [Line, prettypr:format(prettypr:beside(prettypr:text("\t"), erl_prettypr:layout(Do)))]);
		true -> []
